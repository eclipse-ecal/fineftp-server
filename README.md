...
| `FINEFTP_SERVER_BUILD_TESTS` | `BOOL` | `OFF` | Build the the fineftp-server tests. Requires C++17. For executing the tests, curl must be available from the `PATH`. For Windows, additionally Powershell and for Linux / macOS the ftp command is used to test the `STOU` command, that is unsupported by `curl`. |
